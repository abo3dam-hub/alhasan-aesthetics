...
- **framer-motion ← LazyMotion** (`src/main.tsx` + 19 ملفًا): أصبح الاستيراد `{ m as motion }` مع تغليف الجذر بـ `<LazyMotion features={domMax} strict>`؛ محرك الأنيميشن يُحمَّل الآن كقطعة منفصلة غير متزامنة بدل أن يكون ضمن الحزمة الأولية. كل الأنيميشن مطابقة تمامًا.
