...
- **قسم إنستغرام** (`src/components/sections/Instagram.tsx`): شبكة 6 صور قبل التذييل مع تكبير عند التحويم وأيقونة إنستغرام، تُدار من لوحة التحكم (Homepage CMS ← إنستغرام: حتى 6 صور + رابط مخصص اختياري + مفتاح إظهار)؛ يظهر shimmer أثناء التحميل ولا يظهر القسم إن لم تُضبط صور. إصلاح لاحق: محرر اللوحة كان يبقى فارغًا عند أول استخدام (الاستعلام يعيد `null` عند غياب الإعداد)؛ أصبح يُهيَّأ بقيم افتراضية فور حلّ الاستعلام، والقسم العام يُخفى أيضًا عندما يكون الإعداد `null`.
...
- **تعبئة حقول عناوين الأقسام بالنص المعروض فعليًا** (`SectionHeaderEditor`): حقول عناوين أقسام الفيديو/الإجراءات/قبل-بعد/آراء المرضى/الأسئلة كانت تُفتح فارغة رغم أن الموقع يعرض الترجمات الافتراضية. أصبحت تُملأ تلقائيًا بالنص الذي يظهر فعليًا في الواجهة — القيمة المحفوظة في CMS إن وُجدت، وإلا الترجمة الافتراضية العربية/الإنجليزية من ملفات اللغة.
...
- **إطار ذهبي رفيع للصور المهمة** (`.img-frame-gold` في `src/index.css`): حلقة ذهبية عبر `outline` بإزاحة 6px — لا تؤثر على التخطيط ولا تتعارض مع `box-shadow` الموجود؛ مطبقة على صورة الطبيب في About وبطاقات قبل/بعد.
- **توحيد التوهج الذهبي عند التحويم** (`card-glow`): أُضيف لبطاقات/بانرات InformationCard وCTA وصور إنستغرام وحاوية الأسئلة الشائعة وبطاقات إحصائيات About — كان موجودًا على معظم البطاقات.
- **حلقات تركيز ذهبية** (`:focus-visible` عام في `src/index.css`): إطار ذهبي للوصولية بدل الأزرق الافتراضي؛ أنماط المكونات الخاصة (أعلى أولوية) تبقى هي الغالبة حيث عُرّفت.
...
- **مواقع العيادات في لوحة التحكم** (الإعدادات ← مواقع العيادات): 3 عيادات حقيقية قابلة للتعديل (الاسم عربي/إنجليزي، المدينة، العنوان عربي/إنجليزي، الهاتف)؛ تُحفظ ضمن إعدادات الطبيب كمصفوفة `clinics`.
- **بيانات MedicalClinic المنظمة**: استُبدلت عقدة `MedicalBusiness` الواحدة برسم `@graph` من عقد `MedicalClinic` مبنية على بيانات اللوحة (الاسم، العنوان البريدي مع ربط المدينة بالدولة، الهاتف). كل عقدة تحمل `areaServed`: طرطوس وبيروت (كمدن) والعراق وسوريا ولبنان والإمارات (كدول) — الطريقة المعتمدة من غوغل للظهور في بحث مناطق بلا عنوان فيزيائي.
- **hreflang**: وسوم `ar` ← `/ar` و`en` ← `/en` و`x-default` ← `/` تُحقن في `<head>` صفحة الرئيسية.
- **إجبار اللغة من المسار**: زيارة `/ar` تفرض العربية و`/en` تفرض الإنجليزية (كانت اللغة تُقرأ من التخزين المحلي فقط)، لتتطابق اللغة المعروضة مع ما تعده وسوم hreflang.
- **أسئلة شائعة للمناطق المخدومة** (أُضيفت 2026-09-23 عبر لوحة التحكم بجلسة أدمن موثقة): 3 أسئلة ثنائية اللغة منشورة الآن — "هل تستقبلون مرضى من طرطوس / لبنان / العراق؟" — تُشير لعيادات دمشق واللاذقية ودبي مع الحجز عبر واتساب؛ تلتقطها بيانات FAQPage تلقائيًا (إجمالي الأسئلة الآن 9).
...
- **جدول جديد** `serviceTokens` (`src/convex/schema.ts`): الاسم، بصمة المفتاح، النطاق، تاريخ الإنشاء — يُخزَّن فقط بصمة SHA-256 سداسية للمفتاح، ولا يُحفَظ المفتاح الصريح أبدًا في قاعدة البيانات أو الكود.
- **دوال جديدة** (`src/convex/analytics.ts`): `registerServiceToken` (تسجيل/تدوير البصمة — داخلية)، `getServiceTokenHash` (داخلية)، `getBriefingStats` (داخلية — تُرجع بالضبط أرقام الإحاطة ليوم واحد بتوقيت Europe/Berlin: الزيارات، الزوار الفريدون، نقرات واتساب/CTA، أعلى 3 صفحات، أعلى 3 دول؛ الافتراضي = الأمس).
- **نقطة نهاية جديدة** `GET /briefing-analytics` (`src/convex/http.ts`): محمية بترويسة `Authorization: Bearer` مع مقارنة ثابتة الزمن للبصمة والنطاق `briefing-analytics`؛ للقراءة فقط بالبناء (لا كتابة ولا بيانات أخرى)، والمفتاح لا يُسجَّل في السجلات أبدًا؛ معامل `?day=YYYY-MM-DD` اختياري.
